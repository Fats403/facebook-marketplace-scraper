...
async function typePasswordViaTab(page, password, debug = false) {
  try {
    ...
    for (let idx = 0; idx < String(password).length; idx++) {
      const ch = String(password)[idx];
...
async function submitLogin(page, debug = false) {
  let submitted = false;
  try {
    await page.focus('input[name="pass"]').catch(() => {});
    await page.keyboard.press("Enter");
    if (debug) console.log("[login] submitted via Enter on password");
    submitted = true;
  } catch {}

  if (!submitted) {
    try {
      const btn = await page.$('button[name="login"]');
      if (btn) {
        if (debug) console.log("[login] clicking button[name=login]");
  ...
  if (!submitted) {
    try {
      ...
      if (btn) {
        if (debug) console.log("[login] clicking generic submit button");
        await btn.click({ delay: 50 });
        submitted = true;
      }
    } catch {}
  ...
  if (!submitted) {
    if (debug) console.log("[login] falling back to form.requestSubmit");
    ...
    if (
      ...
      try {
        ...
        if (hasEmail && hasPass) {
          ...
          // Type password by moving focus via Tab, then submit using multiple strategies
          const passTyped = await typePasswordViaTab(
            page,
            loginPassword,
            debug
          );
          if (passTyped) {
            await submitLogin(page, debug);
            await Promise.race([
...
function applyPostFilter(listings, queryText, postFilter) {
  if (
  ...
  if (postFilter === "exact") {
    return listings.filter((item) => {
      ...
      const hay = `${name} ${url} ${price}`;
      return hay.includes(q);
    });
  }
  if (postFilter === "fuzzy") {
    return listings.filter((item) => {
      const fields = [item?.name, item?.url, item?.price].map(normalizeText);
      for (const f of fields) {
        if (!f) continue;
        if (f.includes(q)) return true;
        // allow up to 2 edits per 10 chars, minimum 1
        const window = Math.max(q.length, 3);
        const tol = Math.max(1, Math.floor(window / 5));
        // sliding compare windows
        for (let i = 0; i <= Math.max(0, f.length - q.length); i++) {
          const slice = f.slice(i, i + q.length);
          if (levenshtein(slice, q) <= tol) return true;
        }
      }
      return false;
    });
...
export async function runScrape({
  ...
  if (mode === "programmatic") {
    ...
    const listings = applyPostFilter(parsed.listings, q, postFilter);
    return { htmlLength: html.length, json: JSON.stringify({ listings }) };
  }
  ...
  try {
    ...
    const urlObj = new URL(url);
    const q = urlObj.searchParams.get("query") || "";
    const listings = applyPostFilter(obj.listings || [], q, postFilter);
    return { htmlLength: html.length, json: JSON.stringify({ listings }) };
  } catch {
    // If LLM returned non-JSON or unexpected shape, just return as-is
